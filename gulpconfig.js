...
module.exports = {
  ...
  browsersync: {
    ...
    proxy: 'localhost:8888/humdata', // We need to use a proxy instead of the built-in server because WordPress has to do some server-side rendering for the theme to work
    watchOptions: {
  ...
  images: {
    ...
    dist: {
      src: [dist+'**/*(*.png|*.jpg|*.jpeg|*.gif|*.svg|*.eot|*.ttf|*.woff)', '!'+dist+'screenshot.png'], // The source is actually `dist` since we are minifying images in place
      imagemin: {
        optimizationLevel: 7,
        progressive: true,
        interlaced: true
  ...
  scripts: {
    ...
    chunks: { // Chunks are arrays of paths or globs matching a set of source files; this way you can organize a bunch of scripts that go together into pieces that can then be bundled (above)
      ...
      footer: [
        ...
        src+'scripts/learning-module.js',
  ...
  watch: { // What to watch before triggering each specified task; if files matching the patterns below change it will trigger BrowserSync or Livereload
    src: {
      ...
      images:       src+'**/*(*.png|*.jpg|*.jpeg|*.gif|*.svg|*.eot|*.ttf|*.woff)',
      theme:        src+'**/*.php',
      livereload:   build+'**/*'
    },
