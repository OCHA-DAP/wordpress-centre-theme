...
/**
 * Create custom post info HTML
 */
if (!function_exists('uncode_custom_post_info')) {
	function uncode_custom_post_info() {
		$categories = get_the_category();
		$separator = ', ';
		$output = array();
		$cat_output = '';
...
if (!function_exists('uncode_author_info')) {
	function uncode_author_info() {
		...
		$output[] .= (get_the_author_meta('facebook')==='') ? '' : '<a href="http://facebook.com/'.get_the_author_meta('facebook') . '" target="_blank"><i class="fa fa-facebook" aria-hidden="true"></i></a>';
		
		$output[] .= (get_the_author_meta('linkedin')==='') ? '' : '<a href="http://linkedin.com/'.get_the_author_meta('linkedin') . '" target="_blank"><i class="fa fa-linkedin" aria-hidden="true"></i></a>';

		$output[] .= (get_the_author_meta('email')==='') ? '' : '<a href="mailto:'.get_the_author_meta('email').'"><i class="fa fa-envelope" aria-hidden="true"></i></a>';
		
		$output[] .= '</div><p>'.get_the_author_meta('description').'</p></div>';

		return '<div class="post-author">' . implode('', $output) . '</div>';
	}
}
 ...
 * Get menu list
 */
if (!function_exists('uncode_get_menu_list')) {
	function uncode_get_menu_list($menu_name) {
