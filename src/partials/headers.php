	...
	class unheader
	{
		...
		function __construct($args, $page_title = '', $subheading = '')
		{
			...
			switch ($header_type)
			{
               ...
               //                      $content_html .= '<iframe height="350" width="1024" frameborder="0" src="https://data.humdata.org/worldmap?id='.$country_id. '" style="height: 350px; width: 1024px; border: none;" class="featured-image"></iframe>';
							...
							if ($header_title_custom !== 'on') {
								...
								$content_html .= '<h1 class="header-title '.(get_the_ID()) ? 'entryid-'.get_the_ID() : ''.' 1 '.implode(' ', $title_classes).'"'.$data_size.'><span>' . $page_title . '</span></h1><div class="search-container">' . get_search_form(false) . '</div>';
								if (basename(get_page_template()) === 'impactstory.php') {
							    	$content_html .= '</div>';
							    }
							} else {
								...
								{
									$content = '<h1 class="header-title '.(get_the_ID()) ? 'entryid-'.get_the_ID() : ''.' 2 '.implode(' ', $title_classes).'"'.$data_size.'><span>';
			...
			case 'header_uncodeblock':
				...
				$this->html .= '<div id="particles-js"></div><div class="'.$header_no_padding . 'header-wrapper header-uncode-block'. ($header_scroll_opacity === 'on' ? ' header-scroll-opacity' : '') . '">
									' . (isset($header_background_array['back_html']) ? $header_background_array['back_html'] : '');
